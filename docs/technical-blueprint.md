...
> Python 模块化单体 + 独立索引 worker + PostgreSQL/pgvector + Git mirror + Tree-sitter + SCIP/scip-clang + Zoekt + MCP read gateway。
...
**输出**：`RepositorySnapshotReady(repo_id, commit_sha, build_profile)`。
...
- 把一次 snapshot 构建拆成 clone/fetch、文件发现、解析、SCIP、lexical、embedding、验证和发布步骤。
...
- 大文件/编译任务运行在固定容器镜像中，源码只读、默认禁网并限制资源。
...
| Docker/容器隔离 | 固定工具链、限制恶意仓库 | 直接在 API 进程执行解析和构建风险过高 |
...
source files
  ...
  -> compile_commands.json validation
  -> scip-clang index.scip
  -> SCIP protobuf importer
  -> AST + occurrence call candidates
...
| 开源技术 | 直接承担的工作 | 优势 | 边界 |
| --- | --- | --- | --- |
| [Tree-sitter](https://github.com/tree-sitter/tree-sitter) | C/C++ 语法树、函数/类型边界、结构化 chunk | 快、容错、增量、多语言 | 不做类型解析，不声称能准确解析宏和动态调用 |
| [SCIP](https://github.com/scip-code/scip) | 统一保存定义、引用、实现和外部符号 | 语言无关、Protobuf 稳定、可扩展其他语言 | 标准本身不提供完整 caller/callee 图 |
| [scip-clang](https://github.com/sourcegraph/scip-clang) | 基于真实 compilation database 的 C/C++ 精确索引 | Clang 语义精度高，支持跨仓外部符号 | 依赖构建画像、生成文件和编译参数 |

**必须自研**

- SCIP 到内部数据模型的 importer。
- `symbol_occurrence -> logical_symbol` 跨版本映射。
- AST call expression 与 SCIP reference 的组合算法。
- citation 的 blob/byte range/AST anchor 与 re-anchor。
- Kconfig/Makefile/build profile 条件建模。

**调用图策略**

- Phase 0 只生成 `static_call_candidate`，带 `source` 和 `confidence`。
- 直接调用可由 AST + SCIP 较高置信解析。
- 函数指针、宏、回调注册只保存候选，不输出“完整调用链”。
- 后续若真实问题需要，再使用 Clang LibTooling 自研 extractor；不把这项工作提前塞进第一版。
...
OpenGrok 很适合独立源码浏览器和交叉引用，但本项目已有 SCIP 结构层，需要的是可嵌入检索 API。Zoekt 的索引和 API 边界更适合做内部召回服务；OpenGrok保留为效果对照，不进入首版运行依赖。
...
- 代码关系首先来自 SCIP/AST 的确定性数据，不需要 LLM 抽取图。
...
Artifact volume  -> index.scip、解析中间产物、评测报告
...
- ACL、snapshot、build profile 和 citation 是查询正确性的核心，必须显式存在于每一步。
...
| SCIP | 是 | C3 | 代码智能交换格式和 importer 输入 | 检索编排、知识治理 |
| scip-clang | 是 | C3 | C/C++ 定义、引用、实现、外部符号 | 动态调用、所有 Kconfig 组合 |
...
5. C3 scip-clang 生成并导入 SCIP
6. C3 创建 symbol/relation/citation
...
实现 C3：Tree-sitter -> SCIP/scip-clang -> logical symbol -> citation。先解决“证据在哪”，再做语义搜索。

### 第四步：建立正式检索

实现 C4 Zoekt 和 C5 embedding，随后由 C7 做四路召回、RRF、可选 reranker。用黄金问题集决定 Qwen3 embedding/reranker 是否保留。

### 第五步：输出稳定 Context Pack

实现 C8 的 versioned schema、token budget、citation 和 retrieval trace。到这里即使没有 MCP，也能通过 REST/CLI验证核心价值。
...
| 代码结构 | Tree-sitter + SCIP/scip-clang | 某语言无合格 SCIP indexer |
...
- SCIP 是语言无关的定义、引用和实现索引协议；其标准 relationship 不等同于完整调用图：[SCIP schema](https://github.com/scip-code/scip/blob/main/scip.proto)
- scip-clang 基于 Clang 并依赖 compilation database，为 C/C++/CUDA 提供精确索引：[scip-clang](https://github.com/sourcegraph/scip-clang)
