...
当前 schema 标识为 `urn:aiknowledge:schema:context-pack:v1`，版本为 `1.0`。Pydantic 模型位于 `src/aikb/context_pack.py`，可通过以下命令输出 JSON Schema：
...
- Context ID：`context_1ee60c0a5dbd4e5cd34154bf`
- Trace ID：`trace_07a671354aac1843ba2fe5f1`
- 活动 snapshot：`snap_0b0e8c0e71ad7f720c31b8e2`
- 5 条代码证据，4,325 个正文字符，约 1,082 token
- 同一命令重复构建得到相同 ID
- 证据覆盖 `include/linux/sched/task.h:64`、`kernel/sched/idle.c` 和 `kernel/sched/core.c:7961-8027`

## 当前限制与后续兼容

- 当前 retrieval channel 是 SQLite FTS5，只用于本地 bootstrap。
- `team_knowledge` 尚未接入已审核知识条目。
- `partial_visibility` 当前为 `false`；进入团队服务后必须由 ACL 层计算。
- Zoekt、symbol/vector 检索和 RRF 将作为新的 retrieval channel 接入 builder，不改变 Context Pack v1 的核心消费方式。
